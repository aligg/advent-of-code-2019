...
def process_coords(input):
    ...
    for item in input:
        num = int(item[1:])
        if item[0] == "R":
            step_tracker.update({(current_position[0], current_position[1] + j): current_steps + j for j in range(num)})
            current_position[1] += num
            current_steps += num
        elif item[0] == "U":
            step_tracker.update({(current_position[0] + j, current_position[1]): current_steps + j for j in range(num)})
...
def find_closest_manhattan(coords):
    closest = -1
    for c in coords:
        sum = abs(c[0]) + abs(c[1])
        if sum == 0:
            continue
        if sum < closest or closest == -1:
            closest = sum
    return closest

def find_closest_steps(coords, wire1, wire2):
    closest = -1
    for c in coords:
        steps = wire1[c] + wire2[c]
        if steps == 0:
            continue
        if steps < closest or closest == -1:
            closest = steps
    return closest


def run_program(one, two):
    wire1 = process_coords(one)
    wire2 = process_coords(two)
    intersections = find_intersection_points(wire1, wire2)
    print(intersections)
    closest = find_closest_manhattan(intersections)
    print(closest)
    closest_steps = find_closest_steps(intersections, wire1, wire2)
    print(closest_steps)


if __name__ == "__main__":
    run_program(TEST_ONE, TEST_TWO)
    run_program(WIRE_ONE, WIRE_TWO)
